...
class PredictorTests(unittest.TestCase):
    ...
    def test_predict_single_pass(self):
        subs, audio_file_path, voice_probabilities = Undertest(n_mfcc=20).predict_single_pass(
            self.__video_file_path, self.__subtitle_file_path, self.__weights_dir
        )
    ...
    def test_throw_terminal_exception_on_missing_video(self):
        try:
            subs, audio_file_path, _ = Undertest(n_mfcc=20).predict_single_pass(None, self.__subtitle_file_path, self.__weights_dir)
